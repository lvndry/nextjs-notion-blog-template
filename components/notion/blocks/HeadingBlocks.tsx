...
function hasTextContent(items: Array<{ plain_text?: string }> | undefined) {
  return !!(
    items &&
    items.length > 0 &&
    items.some((t) => (t.plain_text ?? "").trim() !== "")
  );
}
...
export function Heading1Block({ block }: { block: NotionBlockWithChildren }) {
  ...
  return (
    <h1
      id={elementId}
      className="text-2xl sm:text-3xl font-bold text-gray-900 dark:text-white mb-4 sm:mb-6 mt-6 sm:mt-8 first:mt-0"
    >
...
export function Heading2Block({ block }: { block: NotionBlockWithChildren }) {
  ...
  return (
    <h2
      id={elementId}
      className="text-xl sm:text-2xl font-semibold text-gray-900 dark:text-white mb-3 sm:mb-4 mt-4 sm:mt-6"
    >
      <RichText items={items} />
    </h2>
  );
...
export function Heading3Block({ block }: { block: NotionBlockWithChildren }) {
  ...
  return (
    <h3
      id={elementId}
      className="text-lg sm:text-xl font-semibold text-gray-900 dark:text-white mb-2 sm:mb-3 mt-4 sm:mt-5"
    >
      <RichText items={items} />
    </h3>
  );
