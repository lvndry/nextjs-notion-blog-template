...
export function ChildPageBlock({ block }: { block: NotionBlockWithChildren }) {
  const { child_page, id } = block as Extract<
  ...
  return (
    <div className="mb-3 sm:mb-4">
      <Link
        href={`/blog/${idNoDash}`}
        className="inline-flex items-center gap-2 px-3 sm:px-4 py-2 sm:py-2.5 bg-gray-100 dark:bg-gray-800 rounded-md text-blue-600 dark:text-blue-400 hover:bg-gray-200 dark:hover:bg-gray-700 text-sm sm:text-base min-h-[44px] transition-colors"
      >
        <svg
